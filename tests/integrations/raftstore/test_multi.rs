...
/* this test uses the failwrite rocksdb option added by shawgerj to lose
 * unflushed memtable data when the kv engine is restarted. It is meant to
 * simulate data lost in a power failure.
 */ 
#[test]
fn test_cluster_restart_failwrite() {
    ...
    cluster.stop_node(1);
    cluster.reboot_engine(1); // not flushed: we lose data here
...
/* this writes k1 and k2 to the cluster, then manually loses data by resetting
 * ApplyState to an earlier index, and deleting k2. k2 should be found again
 * after restarting the node, because the raft log will be reapplied
 */
#[test]
